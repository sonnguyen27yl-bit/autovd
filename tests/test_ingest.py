...
def test_ingest_rejects_invalid_media(tmp_path: Path) -> None:
    ...
    with (
        JobWorkspace(root=tmp_path / "jobs") as workspace,
        pytest.raises(MediaIngestError, match="valid video"),
    ):
...
def test_ingest_rejects_count_limit(tmp_path: Path) -> None:
    ...
    with (
        JobWorkspace(root=tmp_path / "jobs") as workspace,
        pytest.raises(MediaIngestError, match="clip count"),
    ):
...
def test_ingest_rejects_size_limit(tmp_path: Path) -> None:
    ...
    with (
        JobWorkspace(root=tmp_path / "jobs") as workspace,
        pytest.raises(MediaIngestError, match="file size"),
    ):
...
def test_ingest_rejects_duration_limit(tmp_path: Path) -> None:
    ...
    with (
        JobWorkspace(root=tmp_path / "jobs") as workspace,
        pytest.raises(MediaIngestError, match="duration"),
    ):
